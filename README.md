...
页面下载文件统一以“需求名_测试用例”命名，并保留对应格式扩展名（`.xlsx`、`.xmind`、`.md`、`.json`）。
历史任务再次下载也适用；文件名中的非法字符会自动替换，服务器原导出文件和任务记录保留。
下载链接携带当前文件名，且下载响应禁用缓存，避免浏览器复用旧的时间戳文件名。
...
高级配置默认展开，“补充生成要求”旁可选择“用例范围”。默认为“全部用例”，保持原有正常、
异常、边界等完整覆盖；选择“核心流程”时只生成正常条件下完成核心业务目标的主流程用例。
关联代码与补充要求仍可使用，但核心流程模式只用它们细化主流程，不扩展到异常或边界场景。
该选项随任务保存，点击历史任务会回显；生成完成后恢复“全部用例”。API 参数为
`case_scope: "all" | "core"`，省略时按 `"all"` 处理。
